...
import "react-dom";
const Show = ({ loan, instalment, ...props }) => {
  const { emps } = useServerFilter(null, null, null, props.mantri);
  const { status_angsuran } = useServerFilter();
  useState(false);
  const statusSekarang = status_angsuran.filter((item) => item.display == loan.status).reduce((acc, curr) => curr.value, null);
                    ...
                    (item, key) => /* @__PURE__ */ jsxs(
                      "tr",
                      {
                        ...
                        children: [
                          ...
                          /* @__PURE__ */ jsx("td", { className: "px-3 py-2", children: item.danatitipan ? "Ya" : "-" }),
