...
class BatchInputSeeder extends Seeder
{
  ...
  {
    $nasabahRaw = collect(json_decode(file_get_contents(storage_path('kts1sabtu.json'))));
    ...
    $officerGrouping = TransactionLoanOfficerGrouping::where('branch_id', 34)
      ->get()
      ->keyBy('kelompok');
    ...
    $nasabah->chunk(100)->each(function ($batch) use ($officerGrouping, &$batchIndex, $totalBatch) {
      ...
      try {
        ...
        foreach ($batch as $ns) {
          ...
          // 1723 mdn5
          // 1622 mdn2
          $mantri = AppHelper::getMantriNoauth($mantriChoice, 2566);
